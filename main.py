...
async def main(page: ft.Page):
    ...
    async def animar_entrada_dashboard():
        ...
        brigada_key = (page.data or {}).get("brigada_activa")
        brigada_info = BRIGADAS.get(brigada_key, {}) if brigada_key else {}
        brigada_nombre = brigada_info.get("nombre", "Sistema de Brigadas Escolares")
    ...
    def on_brigada_seleccionada(brigada_key: str):
        ...
        if not isinstance(page.data, dict):
            page.data = {}
        page.data["brigada_activa"] = brigada_key  # puede ser None
        
        if brigada_key:
            aplicar_paleta(page, brigada_key)
        else:
            aplicar_paleta_neutra(page)  # Sin brigada → paleta neutra
