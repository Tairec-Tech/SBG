...
def _crear_tarjeta(page: ft.Page, cfg: dict, on_select):
    ...
    color = cfg.get("hex_primario", COLOR_PRIMARIO)
    grad = [cfg.get("hex_primario_oscuro", color), color]
    icono_str = cfg.get("icono", "SHIELD")
    icono_val = getattr(ft.Icons, icono_str, ft.Icons.SHIELD)
    slug = cfg.get("slug", "ecologica")
    nombre = cfg.get("nombre", "Brigada")
    ...
    panel_deslizante = ft.Container(
        content=ft.Column([
            ...
            ft.Text(desc, size=13, color=ft.Colors.with_opacity(0.9, "white"),
    ...
    icon_circle = ft.Container(
        content=ft.Icon(icono_val, color="white", size=44),
    ...
    contenido_base = ft.Container(
        content=ft.Column([
            ...
            ft.Text(nombre, size=22, weight="w800", color=COLOR_TEXTO,
                    text_align=ft.TextAlign.CENTER, font_family="Outfit"),
            ft.Text(sub, size=13, weight="w500",
                    color=ft.Colors.with_opacity(0.7, color),
                    text_align=ft.TextAlign.CENTER, font_family="Outfit") if sub else ft.Container(),
    ...
    btn_ingresar = ft.Container(
        ...
        on_click=lambda _: on_select(slug),
    )
        ...
        content=ft.Stack([
            ft.Container(
                content=ft.Column([
                    ...
                    ft.Icon(icono_val, size=36, color=ft.Colors.with_opacity(0.3, "white")),
                    ft.Container(height=6),
                    ft.Text(nombre, size=18, weight="w700", color="white",
                            text_align=ft.TextAlign.CENTER, font_family="Outfit"),
                    ft.Container(height=10),
                    ft.Divider(height=1, color=ft.Colors.with_opacity(0.3, "white")),
                    ft.Container(height=10),
                    ft.Text(desc_larga, size=13, color=ft.Colors.with_opacity(0.95, "white"),
                            text_align=ft.TextAlign.CENTER, font_family="Outfit"),
                    ft.Container(expand=True),
                    btn_ingresar,
                    ft.Container(height=10),
...
def build(page: ft.Page, on_select) -> ft.Control:
    ...
    rol = str(_data.get("rol", "")).lower()
    es_admin = rol in ["administrador", "admin", "directivo"]

    # Blindaje estricto: Si no es admin, no renderiza la selección.
    if not es_admin:
        import screens.screen_brigades
        return screens.screen_brigades.build(page)
    ...
    # ═══════════════════════════════════════════
    tarjetas = [_crear_tarjeta(page, cfg, on_select) for key, cfg in BRIGADAS.items()]
