...
def get_kpis_estadisticas(tipo_brigada=None, brigada_rol_id=None, institucion_id=None):
    ...
    try:
        ...
        if brigada_rol_id is not None:
            ...
            if tipo_brigada:
                where_b = "WHERE b.Institucion_Educativa_idInstitucion = %(inst_id)s AND b.tipo_brigada = %(tb)s"
                params["inst_id"] = institucion_id
                params["tb"] = tipo_brigada
            else:
                where_b = "WHERE b.Institucion_Educativa_idInstitucion = %(inst_id)s"
                params["inst_id"] = institucion_id
        elif tipo_brigada:
...
def get_actividades_por_mes(tipo_brigada=None, brigada_rol_id=None, institucion_id=None):
...
def get_tendencia_reportes_por_mes(tipo_brigada=None, brigada_rol_id=None, institucion_id=None):
    ...
    if brigada_rol_id is not None:
        ...
        if tipo_brigada:
            where_b = "WHERE b.Institucion_Educativa_idInstitucion = %s AND b.tipo_brigada = %s"
            params = [institucion_id, tipo_brigada, institucion_id, tipo_brigada, institucion_id, tipo_brigada]
        else:
            where_b = "WHERE b.Institucion_Educativa_idInstitucion = %s"
            params = [institucion_id, institucion_id, institucion_id]
    elif tipo_brigada:
...
def get_distribucion_estados_actividades(tipo_brigada=None, brigada_rol_id=None, institucion_id=None):
...
# ==============================================================
# NUEVAS CONSULTAS: Explotación de planificación JSON
# Parsing en Python para compatibilidad con MariaDB 10.4
# ==============================================================

def _fetch_descripciones_actividades(tipo_brigada=None, brigada_rol_id=None, institucion_id=None):
    """Obtiene las descripciones (JSON o texto) de todas las actividades filtradas.
    Se usa internamente para parsear en Python y agregar por campo."""
    sql = """
    ...
    try:
        ...
        print(f"Error obteniendo descripciones para estadísticas: {e}")
        return []


def _agrupar_por_campo_json(descripciones, campo, valor_defecto="Sin clasificar"):
    """Parsea cada descripcion con util_json_plan y agrega conteo por campo."""
    import sys, os
    ...
    for desc in descripciones:
        plan = deserializar_plan(desc)
        valor = (plan.get(campo) or "").strip()
        if not valor:
            valor = valor_defecto
        conteos[valor] = conteos.get(valor, 0) + 1

    # Retornar como lista de dicts ordenada por conteo descendente
    resultado = [{"nombre": k, "conteo": v} for k, v in conteos.items()]
    resultado.sort(key=lambda x: x["conteo"], reverse=True)
    return resultado


def get_actividades_por_momento(tipo_brigada=None, brigada_rol_id=None, institucion_id=None):
    """Agrupa actividades por momento_escolar (parseado desde JSON en Python)."""
    descripciones = _fetch_descripciones_actividades(tipo_brigada, brigada_rol_id, institucion_id)
    if not descripciones:
        return []
    return _agrupar_por_campo_json(descripciones, "momento_escolar", "Sin momento")


def get_actividades_por_origen(tipo_brigada=None, brigada_rol_id=None, institucion_id=None):
    """Agrupa actividades por origen_actividad (parseado desde JSON en Python)."""
    descripciones = _fetch_descripciones_actividades(tipo_brigada, brigada_rol_id, institucion_id)
    if not descripciones:
        return []
    return _agrupar_por_campo_json(descripciones, "origen_actividad", "Sin origen")


def get_actividades_por_nivel(tipo_brigada=None, brigada_rol_id=None, institucion_id=None):
    """Agrupa actividades por nivel_educativo (parseado desde JSON en Python)."""
    descripciones = _fetch_descripciones_actividades(tipo_brigada, brigada_rol_id, institucion_id)
    if not descripciones:
        return []
