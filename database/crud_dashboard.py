...
def get_kpi_stats(tipo_brigada=None, institucion_id=None):
    """
    Retorna un diccionario con estadísticas clave, filtradas por institucion_id o tipo_brigada.
    """
    stats = {
        "total_brigadas": 0,
        "total_usuarios": 0,
    ...
    try:
        if institucion_id:
            sql = """
                SELECT 
                    (SELECT COUNT(*) FROM brigada WHERE Institucion_Educativa_idInstitucion = %s) as total_brigadas,
                    (SELECT COUNT(*) FROM usuario u JOIN brigada b ON u.Brigada_idBrigada = b.idBrigada WHERE b.Institucion_Educativa_idInstitucion = %s) as total_usuarios,
                    (SELECT COUNT(*) FROM actividad a JOIN brigada b ON a.Brigada_idBrigada = b.idBrigada WHERE b.Institucion_Educativa_idInstitucion = %s AND a.estado NOT IN ('Completada', 'Cancelada')) as actividades_activas,
                    (SELECT COUNT(*) FROM actividad a JOIN brigada b ON a.Brigada_idBrigada = b.idBrigada WHERE b.Institucion_Educativa_idInstitucion = %s AND a.estado = 'Completada') as actividades_completadas
            """
            rows, _ = ejecutar(sql, (institucion_id, institucion_id, institucion_id, institucion_id))
        elif tipo_brigada:
            sql = """
                SELECT 
...
def get_activities_stats_by_month(tipo_brigada=None, institucion_id=None):
    """
    Retorna cantidad de actividades por mes, filtradas por institucion_id o tipo_brigada.
    """
    if institucion_id:
        sql = """
