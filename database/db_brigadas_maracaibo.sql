...
CREATE TABLE `brigada` (
  ...
  `tipo_brigada` ENUM('ecologica','riesgo','patrulla','convivencia','auxilios','educacion_fisica','conuco','patrimonio','artes','patriotica','ddhh') NOT NULL DEFAULT 'ecologica',
...
CREATE TABLE `reporte_de_impacto` (
  ...
  `brigada` varchar(100) DEFAULT NULL,
  `area_evaluada` varchar(100) DEFAULT NULL,
...
CREATE TABLE `reporte_actividad` (
  ...
  `participantes` varchar(500) DEFAULT NULL,
  `Actividad_idActividad` int(11) NOT NULL,
  `Usuario_idUsuario` int(11) NOT NULL,
...
ALTER TABLE `brigada`
  ...
  ADD UNIQUE KEY `idx_brigada_profesor_unico` (`profesor_id`),
  ADD UNIQUE KEY `idx_brigada_tipo_inst` (`Institucion_Educativa_idInstitucion`, `tipo_brigada`);
...
ALTER TABLE `turno`
  ...
  ADD KEY `fk_turno_brigada_idx` (`Brigada_idBrigada`);
...
--
ALTER TABLE `turno`
  ADD CONSTRAINT `fk_turno_brigada` FOREIGN KEY (`Brigada_idBrigada`) REFERENCES `brigada` (`idBrigada`) ON DELETE CASCADE ON UPDATE NO ACTION;
